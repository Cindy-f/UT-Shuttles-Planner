class Graph():
    def __init__(self, nodes, links):
        ...
        self.route_matrix = [[[''] for _ in range(self.size)] for _ in range(self.size)]
    ...
    def get_time_matrix(self):
        # use Floyd Warshall algorithm to get minimum time matrix
        # meanwhile update the route matrix 
        size = len(self.nodes)
        ...
        # initialize times based on links 
        for link in self.links:
            i = link.starting_node.num - 1
            j = link.ending_node.num - 1
            the_time = link.time
            shuttle_number = link.shuttle_number 
            if the_time < time_matrix[i][j]:
                time_matrix[i][j] = the_time
                self.route_matrix[i][j][0] = shuttle_number
                self.edges_matrix[i][j][0] = Edge(self.nodes[i], self.nodes[j], the_time)

        # Iterate over nodes
        for k in range(size):
            for i in range(size):
                for j in range(size):
                    dup_time = time_matrix[i][j]
                    time_matrix[i][j] = min(time_matrix[i][j], time_matrix[i][k] + time_matrix[k][j])
                    if dup_time != time_matrix[i][j]:
                        self.edges_matrix[i][j].pop()
                        self.edges_matrix[i][j].append(Edge(self.nodes[i], self.nodes[k], time_matrix[i][k]))
                        self.edges_matrix[i][j].append(Edge(self.nodes[k], self.nodes[j], time_matrix[k][j]))
        return time_matrix
...
def main():
    ...
    bus642 = Bus('642', [node1, node2, node15, node7, node4, node5, node13], [3, 2, 1, 1, 15, 5, 3])
